...
//        System.out.println("action: " + stripsActionStack.peek().toString());
     ...
     */
    public GameState(GameState parent, HashMap<Integer, Peasant> peasants, HashMap<Integer, MyResource> resources, Stack<StripsAction> stripsActionStack) {
        this.parent = parent;
    ...
    public static class MyResource {
        ...
        public MyResource(Position position, int amount, Type type, int id){
            this.amount = amount;
            this.type = type;
        ...
        public MyResource clone() {
            return new MyResource(position, amount, type, id);
        }
    }
    ...
    public HashMap<Integer, MyResource> getResources() {
        HashMap<Integer, MyResource> newMap = new HashMap<>();
    ...
    public List<GameState> generateChildren() {
        ...
        List<StripsAction> availableActions = new ArrayList<>();
        List<GameState> currentGameStateChildren = new ArrayList<>();

        for (MyResource resourceView : getResources().values()) {
            //System.out.println("gonig through resource views");
            for (MyResource resourceView2 : getResources().values()) {
                ...
                for (int k = 1; k <= getNumberOfPeasants(); k++) {
                    ...
                    if (this.getRequiredGold() > 0 && resourceView2.getType() == ResourceNode.Type.GOLD_MINE)
                        availableActions.add(new HarvestGold(resourceView.getPosition(), resourceView2.getPosition(), k, resourceView2.getId()));
                    else if (this.getRequiredWood() > 0 && resourceView2.getType() == ResourceNode.Type.TREE)
                        availableActions.add(new HarvestWood(resourceView.getPosition(), resourceView2.getPosition(), k, resourceView2.getId()));
                }
            }
            ...
            for (int k = 1; k <= getNumberOfPeasants(); k++) {
                if (this.getRequiredGold() > 0 && resourceView.getType() == ResourceNode.Type.GOLD_MINE)
                    availableActions.add(new HarvestGold(new Position(townhallView.getXPosition(), townhallView.getYPosition()), resourceView.getPosition(), k, resourceView.getId()));
                else if (this.getRequiredWood() > 0 && resourceView.getType() == ResourceNode.Type.TREE)
                    availableActions.add(new HarvestWood(new Position(townhallView.getXPosition(), townhallView.getYPosition()), resourceView.getPosition(), k, resourceView.getId()));
            }

            // Going back to townhall
            for (int k = 1; k <= getNumberOfPeasants(); k++) {
                availableActions.add(new Deposit(resourceView.getPosition(), new Position(townhallView.getXPosition(), townhallView.getYPosition()), k, townhallView.getID()));
            }
        }

        availableActions.add(new BuildPeasant());

        for (StripsAction action : availableActions) {
            ...
            if (action.preconditionsMet(this)) {
                currentGameStateChildren.add(action.apply(this));
            }
        }
        ...
        return currentGameStateChildren;
